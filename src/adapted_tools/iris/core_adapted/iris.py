...
from .config import CODEQL_DIR, CODEQL_DB_PATH, PACKAGE_MODULES_PATH, OUTPUT_DIR, ALL_METHOD_INFO_DIR, PROJECT_SOURCE_CODE_DIR, CVES_MAPPED_W_COMMITS_DIR, CODEQL_QUERY_VERSION


from .logger import Logger
from .queries import QUERIES
from .prompts import API_LABELLING_SYSTEM_PROMPT, API_LABELLING_USER_PROMPT
from .prompts import FUNC_PARAM_LABELLING_SYSTEM_PROMPT, FUNC_PARAM_LABELLING_USER_PROMPT

from .codeql_queries import QL_SOURCE_PREDICATE, QL_STEP_PREDICATE, QL_SINK_PREDICATE
from .codeql_queries import EXTENSION_YML_TEMPLATE, EXTENSION_SRC_SINK_YML_ENTRY, EXTENSION_SUMMARY_YML_ENTRY
from .codeql_queries import QL_METHOD_CALL_SOURCE_BODY_ENTRY, QL_FUNC_PARAM_SOURCE_ENTRY, QL_FUNC_PARAM_NAME_ENTRY
from .codeql_queries import QL_SUMMARY_BODY_ENTRY, QL_BODY_OR_SEPARATOR
from .codeql_queries import QL_SUBSET_PREDICATE, CALL_QL_SUBSET_PREDICATE
from .codeql_queries import QL_SINK_BODY_ENTRY, QL_SINK_ARG_NAME_ENTRY, QL_SINK_ARG_THIS_ENTRY

from .modules.codeql_query_runner import CodeQLQueryRunner
from .modules.contextual_analysis_pipeline import ContextualAnalysisPipeline
...
class SAPipeline:
    def __init__(self,
            ...
            # New arguments
            project_source_dir: str = None,
            output_dir: str = None,
            codeql_db_path: str = None,
            project_fixed_methods: pd.DataFrame = None,
            cve_id: str = None,
            cve_fixing_commits: list = None,
    ):
        ...
        if cve_id:
            self.cve_id = cve_id
        elif len(project_name.split("_")) > 3:
            self.cve_id = project_name.split("_")[3]

        # Load some basic information, such as commits and fixes related to the CVE
        if project_source_dir:
            self.project_source_code_dir = project_source_dir
        else:
            self.project_source_code_dir = f"{PROJECT_SOURCE_CODE_DIR}/{self.project_name}"
            
        if cve_fixing_commits is not None:
            self.cve_fixing_commits = cve_fixing_commits
        elif self.cve_id is not None and self.cve_id.startswith("CVE-") and os.path.exists(CVES_MAPPED_W_COMMITS_DIR):
        ...
        if project_fixed_methods is not None:
            self.project_fixed_methods = project_fixed_methods
        elif os.path.exists(ALL_METHOD_INFO_DIR):
            ...
            self.project_fixed_methods = pd.DataFrame(columns=["file"])

        if not self.project_fixed_methods.empty and "file" in self.project_fixed_methods.columns:
            ...
            self.fixed_modules = self.project_fixed_modules \
             ...
             self.fixed_modules = pd.DataFrame(columns=["module"])
        ...
        if codeql_db_path:
            self.project_codeql_db_path = codeql_db_path
        else:
        ...
        if not os.path.exists(f"{self.project_codeql_db_path}/db-java") and not os.path.exists(self.project_codeql_db_path):
            if not self.no_logger:
                self.master_logger.info(f"Processing {self.project_name} (Query: {self.query}, Trial: {self.run_id})...")
...
"""
    ...
    def build_and_save_source_qll_with_source_node(self):
        my_source_content = QL_SOURCE_PREDICATE.format(
            body=f"sourceNode(src, \"{self.project_name}\")", additional="")
        os.makedirs(os.path.dirname(self.source_qll_path), exist_ok=True)
        with open(self.source_qll_path, "w") as f:
    ...
    def build_and_save_sink_qll_with_sink_node(self):
        my_sink_content = QL_SINK_PREDICATE.format(
            body=f"sinkNode(snk, \"{self.project_name}\")", additional="")
        os.makedirs(os.path.dirname(self.sink_qll_path), exist_ok=True)
        with open(self.sink_qll_path, "w") as f:
